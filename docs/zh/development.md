...
产物在 `dist/pixiv-pbd-api.exe`。它和 `python -m pixiv_pbd_manager.gui_api` 接受同样的 argv / `--payload-file` / `-`(stdin),输出同样的 JSON Lines。
...
注意:目前打出来体积比较大(~344 MB),因为 `imagehash` 拖了 scipy + numpy + PyWavelets。Tauri 安装包打包阶段会进一步压缩,最终用户下载的安装包大小预计 ~100 MB 量级。后续若需要进一步减体积,可以考虑自己实现 phash 替代 imagehash + scipy。

P2 会把这个 exe 接到 Tauri sidecar,P3 会让 GitHub Actions 自动出 release 包。
