...
当目录里有多个作品 ID 时，在线解析会从较新、中间和较旧位置抽取有限数量的 PID。成功解析的样本必须属于同一作者；只要发现不同作者，目录就保留在“未识别”列表中供手动处理，不按多数票归属，也不继续用模糊名字覆盖冲突。单个已删除、受限或无法访问的作品不会让整个文件夹解析失败。抽样一致不代表已验证目录内每一张图片。

作品子目录中的数字若与其中图片的作品 ID 一致，不会被当成作者 ID 覆盖外层作者目录。文件名中的明确作品格式（例如 `2026-09-09_12345678_p0.jpg`）优先于日期过滤。同一作者的多个保存目录会分别保留；不同扫描根目录下的同名目录也会分别识别。设置扫描深度时，每个显式选择的根目录都按自己的深度范围扫描，重叠文件只计一次。
...
GUI 可以使用文件名里的作品 ID 在线请求 Pixiv，解析出作者 ID 后通过扫描预览确认写入数据库。离线扫描会依次尝试已有保存路径、数据库中已知的作品归属，以及唯一的完整作者名匹配；完整名字匹配不忽略标点或 `@` 后缀。已知作品归属冲突时不会改用名字猜测；缺少可靠线索的目录保留为“未识别”候选。
...
图库支持按创建时间、修改时间、文件名、文件大小、像素面积（宽×高）和评分升降序排序。文件名采用自然排序（如 `2.jpg` 在 `10.jpg` 前）；主排序相同时按文件名和完整路径稳定排序，缺失时间或尺寸始终排在最后。排序应用于所有筛选结果，切换排序会回到网格顶部，不清除多选。排序偏好保存在桌面端 UI 状态中，“重置所有设置”恢复默认，“重置窗口布局”不影响它。

索引格式 v2 增加可空的 `created_ns`。旧索引仍可直接打开，后台增量扫描会补齐文件创建时间，并保留尺寸缓存、图片标签、Pixiv 标签、收藏、评分和标记。创建时间不等于下载时间或 Pixiv 投稿时间；无法获取时保持未知，不使用其他含义的时间代替。详情页可查看创建和修改时间。
