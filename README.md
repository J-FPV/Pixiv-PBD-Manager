...
- 长任务会在底部状态栏显示进度；下载时显示总进度、当前图片进度和下载速度，并可用红色按钮取消当前任务。
- 复制作者作品页 URL。
...
这里的 `ArtistName` 会被当作候选作者名。勾选“模糊搜索作者名”后，软件会在线搜索 Pixiv 用户并尝试匹配作者 ID。因为手写名称可能不准确，这个功能默认关闭；建议先用较高匹配阈值试扫一小部分目录，确认结果后再大规模使用。
