...
如果默认浏览器不是安装了 Powerful Pixiv Downloader 的浏览器，可以在左侧“浏览器”区域指定 Chrome/Edge 的 exe 路径和用户数据目录。
...
软件会从返回结果里取每一页的 `urls.original` 原图地址，再用 Python 下载文件。下载请求会带上作品页 `Referer`、浏览器风格的 `User-Agent`，以及可选 Pixiv cookie。
